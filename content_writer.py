...
""".strip()
    ...
    def _generate_latest_issue_post(self, topic: dict, campaigns: list) -> str:
        ...
        if self.gemini_enabled:
            ...
            system = "이슈 및 트렌드 전문 카피라이터. 제공된 실시간 뉴스 정보를 바탕으로 정보의 신뢰성과 몰입감을 갖춘 마크다운 포스팅을 작성한다."
            user = f"""
이슈 키워드: {title}
참고 뉴스 정보: {summary}

규칙:
1. 제목(#): 호기심을 유발하고 유용한 가치를 전달하는 매력적인 후킹성 제목 (예: "왜 갑자기 모두가 '{title}'을 주목하나? 사건의 전말과 쟁점 정리")
2. 서론: 해당 키워드가 실시간으로 급상승하고 화제가 된 배경을 뉴스 내용을 바탕으로 자연스럽게 도입.
3. 본문: 제공된 '참고 뉴스 정보'의 헤드라인들을 종합적으로 분석하여, 현재 구체적으로 어떤 일이 벌어졌는지(사건 발단/경과), 대중 및 여론의 다양한 시각(찬반 또는 쟁점), 그리고 향후 전망을 3가지 소제목으로 나누어 설득력 있게 상술. (존재하지 않는 가상의 사실을 무리하게 지어내지 말 것)
